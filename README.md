...
**파스타 이미지에 대한 미분 적용 점수 예시**
모델 출력값이 미분 기반 가중 조정을 거쳐 최종 점수로 변환된다.

![파스타](https://github.com/CHOMINJUN-127/math-2/blob/main/%ED%8C%8C%EC%8A%A4%ED%83%80.png)


**치킨 이미지에 대한 미분 적용 점수 예시**
미분값 변화에 따라 동일한 분류 결과라도 점수 차이가 발생한다.

![치킨](https://github.com/CHOMINJUN-127/math-2/blob/main/%EC%B9%98%ED%82%A8.png)

### 🔹 배고픔 수준 입력에 따른 추천 결과 (대표 예시)

아래 결과는 hunger_level = 6을 입력했을 때의 실행 화면으로,
본 프로젝트에서 구현한 **미분 기반 추천 로직의 동작 과정을 보여주기 위한 대표적인 예시**이다.

해당 경우에는 샐러드가 최종 추천 결과로 출력되었지만,
배고픔 수준 값이나 입력 이미지가 달라질 경우
동일한 계산 과정에 따라 다른 음식이 추천 결과로 선택된다.

즉, 본 예시는 특정 음식 하나에 대한 결과를 강조하기 위한 것이 아니라,
**모든 음식 후보에 대해 동일한 미분 적용 및 점수 비교 과정이 수행됨을 설명하기 위한 예시**이다.

![샐러-딥러닝](https://github.com/CHOMINJUN-127/math-2/blob/main/%EC%83%90%EB%9F%AC%EB%93%9C-%EB%94%A5%EB%9F%AC%EB%8B%9D.png)


**샐러드 이미지에 대한 미분 적용 점수 예시**
사용자 입력 변수와 미분 함수가 결합된 추론 결과를 확인할 수 있다.
